...
const userSchema = new mongoose.Schema({
    ...
    password: {
        type: String,
        // Make password required ONLY if 'provider' is not set (or specifically 'email')
        required: function() {
            // 'this' refers to the document being validated
            // Password not needed if provider is 'google' (or others you might add)
            return !this.provider || this.provider === 'email'; // Adjust 'email' if you use null/undefined for standard signup
        },
        select: false // Keep hidden from default queries
    },
    isAdministrator: {
        type: Boolean,
        required: true,
