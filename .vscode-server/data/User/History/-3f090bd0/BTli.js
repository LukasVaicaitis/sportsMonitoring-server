...
router.post('/generate', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const totalExerciseTarget = prefNumExercises; // Target total from prefs

        // --- Use consistent variable names declared with let ---
        ...
        if (prefMuscleFocus === 'Full Body') {
            // Ensure we use the correct variable names here too
            primaryExerciseCount = 0; // Reset counts if forcing fallback for Full Body
            ...
            secondaryTargetGroups = pairings[primaryTargetGroup];
             if (secondaryTargetGroups.length === 0) {
                 secondaryExerciseCount = 0;
                 primaryExerciseCount = totalExerciseTarget; // Use the corrected name
             } else if (secondaryExerciseCount <= 0 && totalExerciseTarget > primaryExerciseCount) { // Check secondary <= 0
                 secondaryExerciseCount = 1;
                 primaryExerciseCount = totalExerciseTarget - 1; // Use the corrected name
             } else if (secondaryTargetGroups.length > 0 && secondaryExerciseCount > 0) {
            ...
            primaryExerciseCount = totalExerciseTarget; // No pairing defined, use corrected name
            secondaryExerciseCount = 0;
        }
