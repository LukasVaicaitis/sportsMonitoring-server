...
router.post('/generate', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        let primaryTargetGroup = '';
        if (prefMuscleFocus !== 'Auto' && prefMuscleFocus !== 'Full Body' && majorGroups.includes(prefMuscleFocus)) {
            primaryTargetGroup = prefMuscleFocus;
            console.log(`[Generate V3] Using user focus: ${primaryTargetGroup}`);
        } else { // Auto or Full Body focus - use recency
            ...
            console.log(`[Generate V3] Using least recent group: ${primaryTargetGroup}`);
        }

        if (prefMuscleFocus === 'Full Body') {
            console.log(`[Generate V3] Full Body selected, using ${primaryTargetGroup} based on recency as primary.`);
            // Pairings below might still apply, leading to a more balanced workout
        }

        // 5. Determine Secondary/Paired Group(s) & Exercise Counts
        let secondaryTargetGroups = [];
        const totalExerciseTarget = prefNumExercises;
        let primaryExerciseTargetCount = Math.max(1, Math.round(totalExerciseTarget * 0.6)); // At least 1 primary if possible
        let secondaryExerciseTargetCount = totalExerciseTarget - primaryExerciseCount;
        ...
        if (pairings[primaryTargetGroup]) {
            secondaryTargetGroups = pairings[primaryTargetGroup];
            if (secondaryTargetGroups.length === 0) { // Ensure secondary count is 0 if no pairs
                secondaryExerciseCount = 0;
                primaryExerciseCount = totalExerciseTarget; // All exercises for primary group
            } else if (secondaryExerciseCount === 0 && totalExerciseTarget > primaryExerciseCount) {
                // If secondary count ended up as 0 due to rounding but total > primary, add at least 1 secondary
                secondaryExerciseCount = 1;
                primaryExerciseCount = totalExerciseTarget - 1;
            }
        } else {
            primaryExerciseCount = totalExerciseTarget; // No pairing defined, all primary
            secondaryExerciseCount = 0;
        }
        console.log(`[Generate V3] Aiming for up to ${primaryExerciseCount} primary (${primaryTargetGroup}) and ${secondaryExerciseCount} secondary (${secondaryTargetGroups.join('/') || 'None'}) exercises. Total target: ${totalExerciseTarget}.`);
        ...
        console.log(`[Generate V3] Found ${primaryMachinesAvailable.length} available primary machines.`);
        console.log(`[Generate V3] Found ${secondaryMachinesAvailable.length} available secondary machines.`);
        ...
        // Select Primary
        let shuffledPrimary = shuffleArray([...primaryMachinesAvailable]); // Shuffle a copy
        let primarySelectedCount = Math.min(primaryExerciseTargetCount, shuffledPrimary.length); // How many we can actually select
        for (let i = 0; i < primarySelectedCount; i++) {
            if (!selectedMachineIds.has(shuffledPrimary[i]._id.toString())) {
        ...
        let secondaryNeeded = Math.min(remainingSlots, secondaryExerciseTargetCount); // How many secondary can we fit?

        if (secondaryNeeded > 0 && secondaryMachinesAvailable.length > 0) {
            ...
            for (let i = 0; plannedExercises.length < totalExerciseTarget && i < shuffledSecondary.length; i++) {
                // Avoid adding a machine if it was already selected (e.g., compound exercise listed for both)
        ...
        console.log(`[Generate V3] Selected ${plannedExercises.length} exercises.`);

        // 8.
        if (plannedExercises.length === 0) {
            console.log(`[Generate V3] ERROR: No machines found for target groups [${primaryTargetGroup}, ${secondaryTargetGroups.join('/')}] in gym ${userGymId} matching type ${prefWorkoutType}.`);
            return res.status(404).json({ msg: "Could not find any suitable machines matching your preferences in the selected gym." });
        }

        //rep range
        let finalWorkoutType = 'Mixed'; // Determine final type based on selection
        const typesInPlan = [...new Set(plannedExercises.map(e => e.exerciseType))];
        if (typesInPlan.length === 1) finalWorkoutType = typesInPlan[0];
        if (prefWorkoutType !== 'Any' && prefWorkoutType !== 'Mixed') finalWorkoutType = prefWorkoutType; // Prioritize preference if specific
        ...
        console.log(`[Generate V3] Saved generated planned workout ${newWorkout._id} with ${plannedExercises.length} exercises.`);
