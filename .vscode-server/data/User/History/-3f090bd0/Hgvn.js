...
router.post('/generateAI', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        // 3. (Optional) Fetch history summary for recency context
        // const historySummary = "..."; // Simplified summary of last trained dates
        ...
        const geminiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`; // Using Flash model
        ...
        const firstBracketIndex = aiResponseText.indexOf('['); // Find the first '['
        const lastBracketIndex = aiResponseText.lastIndexOf(']'); // Find the last ']'

        if (firstBracketIndex !== -1 && lastBracketIndex !== -1 && lastBracketIndex > firstBracketIndex) {
            // If both found, extract the substring including the brackets
            cleanedJsonString = aiResponseText.substring(firstBracketIndex, lastBracketIndex + 1);
            console.log("[Generate AI] Extracted JSON string using brackets:", cleanedJsonString);
        } else {
            // Fallback if brackets aren't found - maybe AI returned plain text error?
            console.error("[Generate AI] Could not find valid JSON array brackets '[]' in AI response. Attempting direct parse/trim.");
            // Try trimming just in case fences weren't there but whitespace was, or AI gave non-JSON error
            cleanedJsonString = aiResponseText.trim();
            // It will likely fail parsing below, but we try.
        }
        let plannedExercisesRaw = [];
        try {
            // --- ADD LOGGING RIGHT BEFORE PARSE ---
            console.log(`[Generate AI] Attempting JSON.parse on string (length ${cleanedJsonString?.length}). String content between >>> <<<:`);
            console.log(`>>>${cleanedJsonString}<<<`); // Log with delimiters
            // --------------------------------------

            plannedExercisesRaw = JSON.parse(aiResponseText.trim());
            if (!Array.isArray(plannedExercisesRaw)) {
                throw new Error("AI response was not a JSON array.");
            }
            console.log(`[Generate AI] Parsed ${plannedExercisesRaw.length} exercises from AI response.`);
        } catch (parseError) {
