...
router.post('/generate', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const prefs = user.preferences || {};
        // Ensure prefWorkoutType is explicitly handled if not set
        const prefWorkoutType = prefs.workoutType && prefs.workoutType !== 'Any' ? prefs.workoutType : 'Any'; // Treat 'Any' as needing analysis
        const prefMuscleFocus = prefs.muscleFocus || 'Auto';
        const prefNumExercises = prefs.numExercises || 5;
        const prefRepRange = prefs.repRange || '8-12';
        console.log(`[Generate V5] User Prefs - Type: ${prefWorkoutType}, Focus: ${prefMuscleFocus}, Num: ${prefNumExercises}, Reps: ${prefRepRange}`);

        let plannedExercises = [];
        let finalWorkoutType = prefWorkoutType; // Start with preference

        // *** NEW: Handle Cardio Preference Directly ***
        if (prefWorkoutType === 'Cardio') {
            console.log('[Generate V5] Handling specific Cardio request.');
            const cardioMachinesAvailable = await Machine.find({
                gymId: userGymId,
                exerciseType: 'Cardio' // Find only Cardio machines
            }).lean();
            ...
            for (let i = 0; i < countToTake; i++) {
                plannedExercises.push({
                    ...
                    trainedMuscle: shuffledCardio[i].trainedMuscle, // Keep muscle info if available
                    weightUnit: 'kg' // Default or adjust as needed
                    // Cardio typically doesn't have reps/weight, maybe duration? Add later if needed.
                });
            }
            finalWorkoutType = 'Cardio'; // Ensure final type is Cardio
            console.log(`[Generate V5] Selected ${plannedExercises.length} random cardio exercises.`);

        } else {
            // *** Original Logic for Strength, Mixed, Any, Full Body, Specific Muscle ***
            console.log('[Generate V5] Running Strength/Mixed/Focus/Any logic.');
            ...
            const majorGroups = ['Chest', 'Back', 'Legs', 'Shoulders', 'Biceps', 'Triceps', 'Abs'];
            const lastTrained = {};
            majorGroups.forEach(g => lastTrained[g] = new Date('1970-01-01')); // Use Date objects for comparison
            for (const workout of recentWorkouts) {
                 const workoutDate = new Date(workout.date); // Ensure date is a Date object
                for (const exercise of workout.exercises) {
                    const majorGroup = getMajorMuscleGroup(exercise.trainedMuscle);
             ...
             console.log('[Generate V5] Last Trained Dates:', lastTrained);


            // 4. Determine Primary Target Group
            let primaryTargetGroup = majorGroups[0]; // Default
            // If user selected a specific muscle group that we track
            if (prefMuscleFocus !== 'Auto' && prefMuscleFocus !== 'Full Body' && majorGroups.includes(prefMuscleFocus)) {
                primaryTargetGroup = prefMuscleFocus;
                console.log(`[Generate V5] Using user focus: ${primaryTargetGroup}`);
            }
            // If user selected 'Auto' or 'Full Body', find the least recently trained group
                ...
                console.log(`[Generate V5] Using least recent group: ${primaryTargetGroup}`);
            }
             // If prefMuscleFocus is something else (e.g., a specific minor muscle), maybe default or handle differently?
             // Current logic defaults to least recent if focus isn't a known major group.


            // 5. Determine Secondary Group(s) & Target Counts
            let secondaryTargetGroups = [];
            const totalExerciseTarget = prefNumExercises;
            let primaryExerciseCount = Math.max(1, Math.round(totalExerciseTarget * 0.6)); // Default 60% primary
            ...
            if (prefMuscleFocus === 'Full Body') {
                // For Full Body, target the primary (least recent) group, but maybe reduce its count
                // and add exercises from other groups to ensure variety.
                // Simple approach: Target least recent primarily, fill remaining slots randomly?
                console.log(`[Generate V5] Full Body focus selected, targeting least recent: ${primaryTargetGroup}`);
                secondaryTargetGroups = []; // Clear pairings
                primaryExerciseCount = Math.max(1, Math.round(totalExerciseTarget * 0.4)); // Reduce primary focus for full body
                secondaryExerciseCount = totalExerciseTarget - primaryExerciseCount; // Increase secondary/other slots
                 // We'll handle filling these 'secondary' slots later with random non-primary exercises
            } else if (pairings[primaryTargetGroup]) {
                secondaryTargetGroups = pairings[primaryTargetGroup];
                // Adjust counts based on pairings
                if (secondaryTargetGroups.length === 0) {
                    ...
                    if (secondaryExerciseCount > 0) {
                        // Distribute secondary count among paired groups (usually just 1)
                        const countPerSecondaryGroup = Math.max(1, Math.floor(secondaryExerciseCount / secondaryTargetGroups.length));
                        secondaryExerciseCount = countPerSecondaryGroup * secondaryTargetGroups.length; // Actual secondary count
                        primaryExerciseCount = totalExerciseTarget - secondaryExerciseCount; // Adjust primary count
                    } else {
                        primaryExerciseCount = totalExerciseTarget; // Fallback
                    }
                }
            } else {
                // No specific pairing defined for the primary group
                primaryExerciseCount = totalExerciseTarget;
                secondaryExerciseCount = 0;
            }
            console.log(`[Generate V5] Aiming for up to ${primaryExerciseCount} primary (${primaryTargetGroup}) and ${secondaryExerciseCount} secondary/other (${secondaryTargetGroups.join('/') || 'Fillers'}) exercises.`);

            // 6. Find Available Exercises for Target Groups at the Gym
            const queryMachineFilter = { gymId: userGymId };
            // If user selected 'Strength', only fetch Strength machines
            if (prefWorkoutType === 'Strength') {
                queryMachineFilter.exerciseType = 'Strength';
                finalWorkoutType = 'Strength'; // Lock final type
            }
             // If user selected 'Mixed' or 'Any', fetch all types initially
             // We will determine finalWorkoutType later based on selected exercises
            ...
            // Fetch filler machines if needed (for Full Body or if primary/secondary are insufficient)
             const fillerMachinesAvailable = (prefMuscleFocus === 'Full Body' || primaryMachinesAvailable.length < primaryExerciseCount || secondaryMachinesAvailable.length < secondaryExerciseCount)
                ? await Machine.find({
                    ...queryMachineFilter,
                    // Exclude primary and secondary groups already targeted
                    trainedMuscle: { $nin: [primaryTargetGroup, ...secondaryTargetGroups] }
                  }).lean()
                : [];


            console.log(`[Generate V5] Found ${primaryMachinesAvailable.length} available primary, ${secondaryMachinesAvailable.length} secondary, ${fillerMachinesAvailable.length} filler machines.`);

            // 7. Select Exercises Randomly, Respecting Counts
            const selectedMachineIds = new Set(); // Track selected IDs to avoid duplicates
            ...
            // Select Secondary (Paired groups)
            ...
            if (remainingSlots > 0 && secondaryTargetGroups.length > 0 && secondaryMachinesAvailable.length > 0) {
                 ...
                 let secondaryToTake = Math.min(remainingSlots, secondaryExerciseCount); // Max secondary needed based on earlier calc
                 for (let i = 0; plannedExercises.length < totalExerciseTarget && i < shuffledSecondary.length && secondaryToTake > 0; i++) {
                    if (!selectedMachineIds.has(shuffledSecondary[i]._id.toString())) {
                        ...
                        secondaryToTake--; // Decrement count needed for this category
                    }
                 }
            }


            // Select Fillers (For Full Body or if primary/secondary fall short)
             remainingSlots = totalExerciseTarget - plannedExercises.length;
             if (remainingSlots > 0 && fillerMachinesAvailable.length > 0) {
                 console.log(`[Generate V5] Selecting ${remainingSlots} filler exercises.`);
            ...
            console.log(`[Generate V5] Selected ${plannedExercises.length} final exercises for the plan.`);

            // Determine final workout type based on selected exercises if not already set
             if (finalWorkoutType === 'Any' || finalWorkoutType === 'Mixed') {
                 const typesInPlan = [...new Set(plannedExercises.map(e => e.exerciseType))];
                 if (typesInPlan.length === 1) {
                     finalWorkoutType = typesInPlan[0];
                 } else if (typesInPlan.length > 1) {
                     finalWorkoutType = 'Mixed';
                 } else {
                     finalWorkoutType = 'Unknown'; // Should not happen if exercises were selected
                 }
             }
             console.log(`[Generate V5] Final Workout Type set to: ${finalWorkoutType}`);

        } // End of ELSE block for non-Cardio types

        // --- Common Logic for ALL workout types from here ---

        // 9. Add Rep Range Logic (Only applies if final type is Strength or Mixed)
        if (finalWorkoutType === 'Strength' || finalWorkoutType === 'Mixed') {
            plannedExercises = plannedExercises.map(ex => {
                // Only add default reps to Strength exercises within a Strength/Mixed plan
                const defaultReps = ex.exerciseType === 'Strength'
                    ? (prefRepRange === '5-8' ? 6 : prefRepRange === '12-15' ? 12 : 10) // Default based on pref range
                    : undefined; // No default reps for non-Strength types
                // Keep existing repetitions if they exist (e.g., from machine defaults), otherwise apply default
        ...
        // Set time to midday to avoid timezone issues with just date part? Or keep as is?
        planDate.setHours(12, 0, 0, 0);
        // Use ISO string for consistency, but maybe just date part is better?
        // const planDateString = planDate.toISOString().split('T')[0]; // YYYY-MM-DD
        const planDateString = planDate.toLocaleDateString('sv-SE'); // YYYY-MM-DD format
        ...
        const newWorkout = new Workout({
            ...
            workoutType: finalWorkoutType, // Use the determined final type
            isCoachAssigned: false
        });

        // 11. Save & Respond
        await newWorkout.save();
        console.log(`[Generate V5] Saved generated planned workout ${newWorkout._id}.`);
        res.status(201).json(newWorkout); // Send back the created workout plan

    } catch (err) {
        console.error("[Generate V5] Error:", err.message, err.stack);
        res.status(500).send('Server Error');
    }
});
...
router.get(
    ...
    [
        authMiddleware,
        // Expect timestamp in ISO format or Unix millis
        query('since', 'Invalid timestamp format for since parameter').optional().isInt({ min: 0 })
    ],
    async (req, res) => {
        const errors = validationResult(req);
        if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
        ...
        const userId = req.user.id;
        const userIdObject = new mongoose.Types.ObjectId(userId);
        // Get timestamp (in milliseconds) from query, default to 0 if not provided
        const sinceTimestampMillis = req.query.since ? parseInt(req.query.since, 10) : 0;
        const sinceDate = new Date(sinceTimestampMillis);

        console.log(`[FitSync] Fetching completed workouts for user ${userId} since ${sinceDate.toISOString()}`);

        try {
            // Find completed workouts where the *updatedAt* timestamp is after the last sync
            // Using updatedAt is better than date/startTime for detecting changes/newly completed workouts
            const workoutsToSync = await Workout.find({
                userId: userIdObject,
                status: 'completed',
                updatedAt: { $gt: sinceDate } // Find workouts updated *after* the last sync time
            })
            .select('date startTime endTime workoutType exercises') // Select all needed fields
            .sort({ updatedAt: 1 }) // Process oldest first
            .lean();

            console.log(`[FitSync] Found ${workoutsToSync.length} workouts to sync.`);
            res.json(workoutsToSync);

        } catch (err) {
            console.error("Error fetching workouts for Fit Sync:", err.message);
            res.status(500).send('Server Error');
        }
    }
);
