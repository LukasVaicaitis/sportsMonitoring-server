...
router.post('/generateAI', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const historyLimit = 7; // Look at last ~week of completed workouts
        const historyStartDate = subDays(new Date(), 30); // Go back max 30 days
        const historyStartDateString = format(historyStartDate, 'yyyy-MM-dd');

        const recentWorkouts = await Workout.find({
            userId: userIdObject,
            status: 'completed',
            date: { $gte: historyStartDateString }
        })
        .sort({ date: -1 })
        .limit(historyLimit)
        .select('date exercises.trainedMuscle') // Only need muscle and date
        .lean();
        console.log(`[Generate AI V2] Found ${recentWorkouts.length} recent completed workouts for history analysis.`);

        const majorGroups = ['Chest', 'Back', 'Legs', 'Shoulders', 'Biceps', 'Triceps', 'Abs'];
        const lastTrained = {}; // Store YYYY-MM-DD string
        majorGroups.forEach(g => lastTrained[g] = 'Never'); // Initialize

        for (const workout of recentWorkouts) { // Iterates newest to oldest
            for (const exercise of workout.exercises) {
                ...
                if (majorGroups.includes(majorGroup) && lastTrained[majorGroup] === 'Never') {
                    // Since workouts are sorted newest first, the first time we see a group,
                    // it's the most recent time it was trained.
                    lastTrained[majorGroup] = workout.date;
                }
            }
        }
        const historySummary = majorGroups.map(g => `${g}: ${lastTrained[g]}`).join(', ');
        console.log(`[Generate AI V2] History Summary (Last Trained): ${historySummary}`);
        // -------------------------------------------
