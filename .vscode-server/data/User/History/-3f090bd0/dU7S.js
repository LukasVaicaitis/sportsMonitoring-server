...
router.get(
    ...
    async (req, res) => {
        ...
        try {
            ...
            const mongoQuery = { userId: userIdObject }; // Base filter: always for the logged-in user

            // Add date filter ONLY if year and month are provided
            if (year && month) {
                const datePrefix = `${year}-${String(month).padStart(2, '0')}-`;
            ...
            if (status) {
                ...
                console.log(`[GET /workouts] Filtering by status: ${status}`);
            } else {
                // Default behaviour if no status specified: maybe return only 'completed'?
                // Or return both? Let's return only 'completed' by default if fetching history
                // UNLESS isCoachAssigned filter is used (then we might want planned coach ones)
                if (isCoachAssigned === undefined) { // If not specifically looking for coach plans
                     mongoQuery.status = 'completed'; // Default to completed for general history view
                     console.log(`[GET /workouts] No status specified, defaulting to: completed`);
                }
            }

            // Add coach assignment filter ONLY if provided
