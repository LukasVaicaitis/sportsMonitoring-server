...
router.post('/generateAI', authMiddleware, async (req, res) => {
    ...
    try {
       ...
       const majorGroups = ['Chest', 'Back', 'Legs', 'Shoulders', 'Biceps', 'Triceps', 'Abs'];
        ...
        // Create performance summary string, filtering out groups with no recent data
        const performanceSummary = Object.entries(lastPerformance)
                                     .map(([group, perf]) => perf) // Get the performance strings
                                     .join('; ');
